...
%% Main loop
for i = 1:nFrames
    img1Path = sprintf('%s/cam_1_%d.tif', dirPath, i);
    img2Path = sprintf('%s/cam_2_%d.tif', dirPath, i);
    img1 = imread(img1Path, 'tiff'); 
    img2 = imread(img2Path, 'tiff');

%% Detect particles on left and right
    [particles1, particles2] = detectParticles(img1, img2);
...
%% Predict using Kalman filter
    for j=1:height(tracks)
        ...
        tracks.trace{j} = tracks.trace{j}.addDetected(predictedCentroid);
...
%% Assign detection to current tracks   
    ...
    areaRatio = zeros(nTracks, nDetections);
...
% Create new tracks of particles not assigned (appeared or false alarm)
    ...
    for j=1:size(centroids, 1)
        ...
        kalmanFilter = configureKalmanFilter('ConstantVelocity', centroid, [300 20], [20, 5], 20);
        trace = particleTrace;
        trace = trace.addDetected(centroid);
...
% Triangulate particles for assigned and new tracks
    ...
    trackedParticlesData = tracks(trackedParticlesIds, :);
...
trackPath = sprintf('%s/tracksComputed.mat', dirPath);
save(trackPath, 'tracks');
